...
@inline function init_arwm(x0::T, algorithm, rng, q, L, all_levels, nX,
                           log_p, log_pr; wrap::Function=identity) where {FT <: AbstractFloat,
                           T <: AbstractVector{FT}}
    d = length(x0)
    # RWM states, adaptation states...
    R = [RWMState(x0, rng, q, wrap) for i = 1:L]
...
function adaptive_rwm_distributed(x0s :: AbstractVector{T}, log_p :: Function, n :: Int, nchains :: Int;
    wrap!::Function=identity, verbose::Bool=false, algorithm::Union{Symbol,Vector{<:AdaptState}}=:ram,
    ...
    iterator = (verbose) ? ProgressBar(1:nchains, printing_delay = 1) : 1:nchains
    Threads.@threads for c in iterator 
        chains[c] = adaptive_rwm(x0s[c], log_p, n; wrap! = wrap!, algorithm,
        thin, b, fulladapt, Sp, Rp, indp,
        q, L, log_pr, all_levels, acc_sw, swaps,
        rng)
    end
    return chains
end 
...
function adaptive_rwm(x0::T, log_p::Function, n::Int; wrap!::Function=identity,
    verbose::Bool=false, algorithm::Union{Symbol,Vector{<:AdaptState}}=:ram,
    thin::Int=1, b::Int=max(1,Int(floor(n/5))), fulladapt::Bool=true, 
    Sp=nothing, Rp=nothing, indp=nothing,
    q::Function=randn!, L::Int=1, log_pr::Function = (x->zero(FT)),
    all_levels::Bool=false, acc_sw::FT = FT(0.234), swaps::Symbol = :single,
    ...
    X, D, R, S, P = init_arwm(x0, algorithm, rng, q, L,
                              all_levels, nX, log_p, log_pr; wrap = wrap!)
...
function adaptive_rwm_(X, D, R, S, P, args, params, x0::T, log_p::Function, n::Int,
    ...
    rng::AbstractRNG; verbose::Bool = false) where {FT <: AbstractFloat,
    T <: AbstractVector{FT}}
    ...
    iterator = (verbose) ? ProgressBar(1:n, printing_delay = 1) : 1:n
    for k in iterator
